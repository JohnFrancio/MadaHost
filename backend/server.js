// backend/server.js - CORRECTION IMPORT REDIS
...
// ✅ CORRECTION: Import Redis avec la nouvelle syntaxe
...
const initRedis = async () => {
  if (process.env.REDIS_URL) {
    try {
      ...
      // ✅ Créer le store APRÈS connexion
      ...
      console.log("✅ Redis Store configuré pour les sessions");
      return true;
    } catch (error) {
      console.error("❌ Erreur initialisation Redis:", error.message);
      console.warn("⚠️  Fallback vers MemoryStore");
      redisClient = null;
      redisStore = null;
      return false;
    }
  } else {
    console.warn("⚠️  REDIS_URL non configuré");
    console.warn(
      "⚠️  Utilisation de MemoryStore (NON RECOMMANDÉ en production)"
    );
    return false;
  }
};
...
// ========================================
// ✅ Initialiser Redis AVANT session
// ========================================
const startServer = async () => {
  ...
  if (redisStore) {
    ...
    console.log("✅ Sessions configurées avec Redis");
  } else {
    console.warn("⚠️  Sessions configurées avec MemoryStore (non recommandé)");
  }
  ...
  server.listen(PORT, async () => {
    ...
    console.log("\n🔴 Session Store:");
    console.log(`  - Type: ${redisStore ? "Redis ✅" : "Memory ⚠️"}`);
    if (redisClient) {
      console.log(
